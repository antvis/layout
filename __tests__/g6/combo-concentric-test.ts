import { CircularLayout, ComboCombinedLayout, ForceAtlas2Layout, GridLayout, Model, RadialLayout } from '../../src';
...
describe('combo concentric force layout', () => {
  ...
  simpleData2.nodes.forEach((node: any) => {
    node.label = node.id;
  });
  // 借助 G6 得到 comboTrees
  ...
  it.only('forceAtlas2 consistancy', () => {
    ...
    graph.on('canvas:click', e => {
      concentricData.nodes.forEach((node: any, i) => {
  ...
  it('default outer + radial inner', () => {
    ...
    graph.on('aftercollapseexpandcombo', e => {
      layout.layout(graph.save() as Model);
      graph.refreshPositions();
    });
  });
