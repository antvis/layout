...
| ordering    | The ordering method for nodes. `null` means nodes are arranged in data order; `'topology'` means in topology order; `'topology-directed'` means in directed topology order; `'degree'` means in degree order. | `null \| 'topology' \| 'topology-directed' \| 'degree'`                      | `null`                    |
...
| nodeSize    | The size of nodes. Used with `nodeSpacing` to calculate radius automatically.                                                                                                                                 | `number \| [number, number] \| ((node: Node) => number \| [number, number])` | `undefined`               |
| nodeSpacing | The spacing between nodes. Used with `nodeSize` to calculate radius automatically. If specified, `startRadius` and `endRadius` will be ignored.                                                               | `number \| ((node: Node) => number)`                                         | `undefined`               |
