...
use super::graph::csr::{prefix_sum, Csr, SwapCsr, Target};
use super::index::Idx;
use super::{
...
pub trait ForEachNodeParallelOp<NI: Idx> {
    ...
    ///
    /// graph.for_each_node_par(&mut node_values, |g, node, node_state| {
...
pub trait ForEachNodeParallelByPartitionOp<NI: Idx> {
    ...
    ///
    /// graph.for_each_node_par_by_partition(&partition, &mut node_values, |g, node, node_state| {
    ///     *node_state = g.out_degree(node);
    /// });
    ///
    /// assert_eq!(node_values[0], 2);
...
pub trait ToUndirectedOp {
    ...
    /// use graph_builder::prelude::*;
    ///
    /// let graph: DirectedCsrGraph<u32> = GraphBuilder::new().edges(vec![(0, 1), (2, 0)]).build();
    ///
