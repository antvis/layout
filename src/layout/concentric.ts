...
export class ConcentricLayout extends Base {
  ...
  public execute() {
    ...
    // sort nodes by value
    layoutNodes.sort((n1: INode, n2: INode) => (n2 as any)[self.sortBy] - (n1 as any)[self.sortBy])

    self.maxValueNode = layoutNodes[0]

    self.maxLevelDiff = self.maxLevelDiff || (self.maxValueNode as any)[self.sortBy] / 4
    ...
    layoutNodes.forEach((node) => {
      if (currentLevel.length > 0) {
        const diff = Math.abs(currentLevel[0][self.sortBy] - (node as any)[self.sortBy])
