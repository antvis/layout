...
export class DagreLayout extends Base {
  ...
  /** 节点水平间距(px) */
  public nodesepFunc: ((d?: SafeAny) => number) | undefined;

  /** 每一层节点之间间距 */
  public ranksepFunc: ((d?: SafeAny) => number) | undefined;
  ...
  public getDefaultCfg(): DagreLayoutOptions {
    return {
  ...
  public layoutNode = (nodeId: string): boolean => {
    const self = this;
    const { nodes } = self;
  ...
  public execute(): { nodes: OutNode[]; edges: Edge[] } {
    ...
    const edges = (self.edges as SafeAny[]) || [];
    ...
    if (radial) {
      ...
      const processNodes = (
        ...
        layerNodes.forEach((node: SafeAny) => {
          const coord = g.node(node);
      ...
      g.edges().forEach((edge: SafeAny) => {
        ...
        if (self.edgeLabelSpace && self.controlPoints && edges[i].type !== 'loop') {
          ...
          controlPoints.forEach((point: SafeAny) => {
      ...
      g.edges().forEach((edge: SafeAny) => {
        ...
        if (self.edgeLabelSpace && self.controlPoints && edges[i].type !== 'loop') {
          ...
          edges[i].controlPoints.forEach((point: SafeAny) => {
            point.x += dBegin[0];
            point.y += dBegin[1];
          });
        }
