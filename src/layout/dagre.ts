...
export class DagreLayout extends Base {
  ...
  public execute(): { nodes: OutNode[]; edges: Edge[] } {
    ...
    let nodeSizeFunc: (d?: OutNode) => number[];
    if (!nodeSize) {
      nodeSizeFunc = (d: OutNode) => {
        if (d.size) {
          if (isArray(d.size)) {
            return d.size;
          }
    ...
    let horisep = getFunc(self.nodesep, 50, self.nodesepFunc);
    let vertisep = getFunc(self.ranksep, 50, self.ranksepFunc);
