...
export class DagreCompoundLayout extends Base {
  ...
  public getDefaultCfg(): DagreCompoundLayoutOptions {
    return {
  ...
  public init(data: ModelType): void {
  ...
  private getLayoutConfig(): {
    graphDef: HierarchyGraphDef;
    graphOption: HierarchyGraphOption;
    graphSettings: DeepPartial<LayoutConfig>;
  } {
    const self = this;
    ...
    const [paddingTop, paddingRight, paddingBottom, paddingLeft] = combos.find(c => !c.collapsed)?.padding || [
      20, 20, 20, 20
    ];
    const graphDef = {
      compound,
  ...
  private getBegin(
    flattenedNodes: Array<HierarchyBaseNodeInfo | HierarchyGraphNodeInfo>,
    ...
    flattenedNodes.forEach(node => {
      ...
      if (type === HierarchyNodeType.META && id !== ROOT_NAME) {
        ...
        if (!node.expanded) {
          ...
          combos[i].padding = [
            subSceneMeta?.paddingTop || 20,
            subSceneMeta?.paddingRight || 20,
            subSceneMeta?.paddingBottom || 20,
            subSceneMeta?.paddingLeft || 20
          ];
        }
    ...
    if (controlPoints) {
      ...
      edges.forEach(sourceEdge => {
        ...
        if (targetNode?.type === NodeType.META) {
          ...
          if (
            !combos[i] ||
            combos[i].inEdges?.some(inEdge => inEdge.source === sourceNode!.id && inEdge.target === targetNode!.id)
          ) {
            return;
          }
        ...
        if (sourceNode?.type === NodeType.META) {
          ...
          if (
            !combos[i] ||
            combos[i].outEdges?.some(oedge => oedge.source === sourceNode!.id && oedge.target === targetNode!.id)
          ) {
            return;
          }
   ...
   * @private
   */
  private getDataByOrder(list: SafeAny[]): SafeAny[] {
