...
function calcNodePair(nodeA: IMysqlNode, nodeB: IMysqlNode): number {
  // 确定两个节点间是否存在连线
...
function calcEnergy(nodes: SafeAny): number {
  let energy = 0;
...
function shuffle(nodes: IMysqlNode[], edges: IEdge[]): number {
...
// 计算节点的能量，
function calcNodeEnergy(node: IMysqlNode, nodes: IMysqlNode[]): number {
  let e = 0.0;
...
function layout(nodes: IMysqlNode[], edges: IEdge[]): { nodes: IMysqlNode[]; edges: IEdge[] } {
  if (nodes.length === 0) {
    return { nodes, edges };
  }
