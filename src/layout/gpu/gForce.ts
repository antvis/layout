...
export class GForceGPULayout extends Base {
  ...
  /** 理想边长 */
  public linkDistance: number | ((d?: SafeAny) => number) | undefined = 1;
  ...
  public getDefaultCfg(): GForceGPULayoutOptions {
    return {
      type: 'gForce-gpu',
      maxIteration: 2000,
      gravity: 10,
  ...
  public executeWithWorker(canvas?: HTMLCanvasElement, ctx?: SafeAny): void {
    const self = this;
    const nodes = self.nodes;
    const center = self.center;
  ...
  public async run(canvas?: HTMLCanvasElement, ctx?: SafeAny): Promise<void> {
    ...
    self.linkDistance = proccessToFunc(self.linkDistance) as (d?: SafeAny) => number;
    self.edgeStrength = proccessToFunc(self.edgeStrength) as (d?: SafeAny) => number;
    ...
    const center = self.center;
    nodes.forEach((node, i) => {
      masses.push((self.getMass as (d?: SafeAny) => number)(node));
      nodeStrengths.push((self.nodeStrength as (d?: SafeAny) => number)(node));
